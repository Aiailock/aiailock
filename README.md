...
- **19. Pocket Book Reader:** an always-populated romantic GIF shelf, short multi-query Commons search, portrait page-turn reading with left swipes and pixel arrows, vertical screen-by-screen pages, safe long-text pagination and device-local reading-mode memory. This is a frontend-only release and adds no database migration.
- **20. Real Paper Book:** two independent phone book modes — a horizontal right-to-left sheet and a vertical bottom-to-top sheet — with a two-sided 3D page, paper grain, moving fold, spine, stacked edges, live shadows and the exact reverse motion. The legacy `book` preference migrates automatically and long pages keep safe internal scrolling.
