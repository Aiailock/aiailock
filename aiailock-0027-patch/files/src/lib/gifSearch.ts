...
export function gifSearchTerms(query: string): string[] {
  ...
  if (includesAny(source, [/обним/, /объят/, /hug/])) add('hug', 'Movicons hug');
  ...
  if (includesAny(source, [/страх/, /страш/, /испуг/, /боюсь/, /fear/, /scared/, /afraid/])) add('scared reaction', 'fear animation');
  if (includesAny(source, [/злост/, /злой/, /бесит/, /ярост/, /anger/, /angry/])) add('angry reaction', 'anger animation');
  if (includesAny(source, [/плач/, /слез/, /рыда/, /cry/, /tears/])) add('crying reaction', 'tears animation');
  if (includesAny(source, [/радост/, /счаст/, /ура/, /happy/, /joy/])) add('happy reaction', 'joy animation');
  if (includesAny(source, [/шок/, /удив/, /офиг/, /wow/, /shock/, /surpris/])) add('shocked reaction', 'surprise animation');
  if (includesAny(source, [/стыд/, /нелов/, /кринж/, /awkward/, /cringe/])) add('awkward reaction', 'facepalm animation');
  if (includesAny(source, [/устал/, /сонн/, /tired/, /exhausted/])) add('tired reaction', 'sleepy animation');
  if (includesAny(source, [/извин/, /прости/, /винов/, /sorry/, /apology/])) add('sorry reaction', 'apology animation');
  if (includesAny(source, [/привет/, /здравств/, /hello/, /hi/])) add('hello animation', 'waving reaction');
  if (includesAny(source, [/пока/, /до свид/, /bye/, /goodbye/])) add('goodbye animation', 'waving reaction');
  if (includesAny(source, [/день рождения/, /birthday/])) add('happy birthday', 'birthday animation');
  if (includesAny(source, [/праздн/, /побед/, /celebrat/, /victory/])) add('celebration reaction', 'victory animation');
  if (includesAny(source, [/кофе/, /coffee/])) add('coffee animation', 'coffee reaction');
  if (includesAny(source, [/работ/, /work/])) add('work reaction', 'busy animation');
  ...
  if (terms.length === 0 && source.length >= 2 && source.length <= 60) add(source.split(/\s+/).slice(0, 3).join(' '));
  if (terms.length === 0 || /[а-яё]/i.test(source)) add('reaction animation', 'animated character');
