...
export default function EffectsLayer({ decorations, seed = 1, gifUrl, disabled = false }: { decorations?: string[] | null; seed?: number; gifUrl?: string | null; disabled?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!root || disabled || reduced || list.length === 0) return;
    const observer = new IntersectionObserver(([entry]) => setActive(Boolean(entry?.isIntersecting)), { rootMargin: '320px 0px' });
    observer.observe(root);
    return () => observer.disconnect();
  }, [disabled, list.length, reduced]);

  if (disabled || reduced || list.length === 0) return null;

  return (
    <div ref={rootRef} aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
      {active && list.includes('petals') && petalRands.map((r, i) => (
      ...
      {active && list.includes('confetti') && confettiRands.map((r, i) => (
      ...
      {active && list.includes('snow') && snowRands.map((r, i) => {
        const size = 3 + r * 4;
