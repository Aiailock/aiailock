...
import { Eye, Gift, Heart, Image as ImageIcon, Mail, RotateCcw, Sparkles, Star } from 'lucide-react';
...
export default function InteractiveMoment({ kind, title, text, row, token, dark, fontClass }: Props) {
  ...
  const options = Array.isArray(row.metadata?.options) ? row.metadata.options.map(String).slice(0, 2) : ['Да', 'Очень'];
  const results = Array.isArray(row.metadata?.results) ? row.metadata.results.map(String).slice(0, 2) : [text, text];
    ...
    <div className="mx-auto max-w-[360px] py-2 text-center">
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            {choiceMode && selected === null && <div className="grid gap-2"><p className="mb-2 text-xs uppercase tracking-[2px] opacity-45">выбери ответ</p>{options.map((option, index) => <motion.button whileTap={reduced ? undefined : { scale: .97 }} type="button" key={`${option}-${index}`} onClick={() => { setSelected(index); remember({ selected: index, complete: true }); }} className="rounded-2xl border border-gold/25 bg-gold/10 px-4 py-3 font-serif text-lg text-gold">{option}</motion.button>)}</div>}
            ...
            {completed && <motion.div initial={reduced ? undefined : { opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }}>
