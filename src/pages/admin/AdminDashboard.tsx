...
function TimelinePanel({ refreshKey }: { refreshKey: number }) {
  ...
  const load = useCallback(async () => {
    const currentRun = ++loadRun.current;
    const search = filter.trim();
    setLoadingRows(true);
    setLoadError("");
    try {
      let els: TimelineRow[] = [];
      let foundMessages: MessageRow[] = [];
      let foundMemories: MemoryRow[] = [];
      let foundScreenshots: ScreenshotRow[] = [];
      let resolvedPreviousBoundary: TimelineRow | null = null;
      let resolvedNextBoundary: TimelineRow | null = null;

      if (search) {
        const pattern = `%${search}%`;
        const messageFields = ["original_text", "display_text"] as const;
        const memoryFields = ["title", "body"] as const;
        const screenshotFields = ["title", "description", "caption"] as const;
        const [messageResults, memoryResults, screenshotResults] = await Promise.all([
          Promise.all(messageFields.map((field) => supabase.from("messages").select("id,sent_at,sender_name,original_text,display_text").eq("is_system_message", false).ilike(field, pattern).limit(250))),
          Promise.all(memoryFields.map((field) => supabase.from("memories").select("id,title,body,occurred_at,importance,place_after_message_id,photo_storage_path,style,metadata").ilike(field, pattern).limit(250))),
          Promise.all(screenshotFields.map((field) => supabase.from("screenshots").select("id,storage_path,title,description,caption,occurred_at,place_after_message_id,animation,position,style,collection_id,collection_order,collection_layout,reaction_emoji,reaction_text").ilike(field, pattern).limit(250))),
        ]);
        const firstError = [...messageResults, ...memoryResults, ...screenshotResults].find((result) => result.error)?.error;
        if (firstError) throw firstError;
        foundMessages = Array.from(new Map(messageResults.flatMap((result) => (result.data ?? []) as MessageRow[]).map((item) => [item.id, item])).values());
        foundMemories = Array.from(new Map(memoryResults.flatMap((result) => (result.data ?? []) as MemoryRow[]).map((item) => [item.id, item])).values());
        foundScreenshots = Array.from(new Map(screenshotResults.flatMap((result) => (result.data ?? []) as ScreenshotRow[]).map((item) => [item.id, item])).values());

        const findElements = async (column: "message_id" | "memory_id" | "screenshot_id", ids: string[]) => {
          const batches: string[][] = [];
          for (let index = 0; index < ids.length; index += 75) batches.push(ids.slice(index, index + 75));
          const results = await Promise.all(batches.map((idsBatch) => supabase
            .from("timeline_elements")
            .select("id,type,occurred_at,display_order,message_id,media_id,memory_id,screenshot_id,style,mood,importance,is_published,visible_from,metadata")
            .in(column, idsBatch)));
          const error = results.find((result) => result.error)?.error;
          if (error) throw error;
          return results.flatMap((result) => (result.data ?? []) as TimelineRow[]);
        };

        const [messageElements, memoryElements, screenshotElements] = await Promise.all([
          findElements("message_id", foundMessages.map((item) => item.id)),
          findElements("memory_id", foundMemories.map((item) => item.id)),
          findElements("screenshot_id", foundScreenshots.map((item) => item.id)),
        ]);
        els = Array.from(new Map([...messageElements, ...memoryElements, ...screenshotElements].map((item) => [item.id, item])).values())
        ...
        els = all.slice(offset, offset + pageSize);
        resolvedPreviousBoundary = page > 0 ? all[0] ?? null : null;
        resolvedNextBoundary = all[offset + pageSize] ?? null;

        const messageIds = els.map((row) => row.message_id).filter((id): id is string => Boolean(id));
        const memIds = els.map((row) => row.memory_id).filter((id): id is string => Boolean(id));
        const shotIds = els.map((row) => row.screenshot_id).filter((id): id is string => Boolean(id));
        const [{ data: msgs, error: messagesError }, { data: mems, error: memoriesError }, { data: shots, error: screenshotsError }] = await Promise.all([
          messageIds.length ? supabase.from("messages").select("id,sent_at,sender_name,original_text,display_text").in("id", messageIds) : Promise.resolve({ data: [] as MessageRow[], error: null }),
          memIds.length ? supabase.from("memories").select("id,title,body,occurred_at,importance,place_after_message_id,photo_storage_path,style,metadata").in("id", memIds) : Promise.resolve({ data: [] as MemoryRow[], error: null }),
          shotIds.length ? supabase.from("screenshots").select("id,storage_path,title,description,caption,occurred_at,place_after_message_id,animation,position,style,collection_id,collection_order,collection_layout,reaction_emoji,reaction_text").in("id", shotIds) : Promise.resolve({ data: [] as ScreenshotRow[], error: null }),
        ]);
        if (messagesError || memoriesError || screenshotsError) throw messagesError ?? memoriesError ?? screenshotsError;
        foundMessages = (msgs ?? []) as MessageRow[];
        foundMemories = (mems ?? []) as MemoryRow[];
        foundScreenshots = (shots ?? []) as ScreenshotRow[];
      }

      if (currentRun !== loadRun.current) return;
      setPreviousBoundary(resolvedPreviousBoundary);
      setNextBoundary(resolvedNextBoundary);
      setRows(els);
      setMessages(foundMessages);
      setMemMap(new Map(foundMemories.map((item) => [item.id, item])));
      setShotMap(new Map(foundScreenshots.map((item) => [item.id, item])));
    } catch (error) {
      if (currentRun === loadRun.current) setLoadError(error instanceof Error ? error.message : "История не загрузилась.");
    } finally {
      if (currentRun === loadRun.current) setLoadingRows(false);
    }
  }, [filter, page]);
  useEffect(() => {
    const timer = window.setTimeout(() => void load(), filter.trim() ? 280 : 0);
  ...
  const visible = rows.filter((r) => {
    ...
    const memory = r.memory_id ? memMap.get(r.memory_id) : null;
    const screenshot = r.screenshot_id ? shotMap.get(r.screenshot_id) : null;
    const haystack = [
      r.type,
      r.mood,
      m?.original_text,
  ...
  return (
    <section ref={sectionRef} className="scroll-mt-20 rounded-[28px] border border-black/5 bg-white/90 p-4 shadow-sm sm:p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <label className="flex w-full max-w-sm items-center gap-2 rounded-xl border border-black/10 bg-white px-3">
          <Search size={15} className="shrink-0 text-burgundy/45" />
          <input
            value={filter}
            onChange={(e) => {
            ...
            placeholder="Найти любой текст во всей истории…"
            className="min-w-0 flex-1 border-0 py-2.5 text-sm outline-none"
          />
          {filter && <button type="button" aria-label="Очистить поиск" onClick={() => setFilter("")} className="rounded-lg p-1 text-burgundy/45"><X size={14}/></button>}
        </label>
      </div>
      {q ? <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-burgundy/10 bg-burgundy/[.035] px-3 py-2 text-xs text-burgundy"><span>{loadingRows ? "Ищу по всей истории…" : `Найдено: ${visible.length}`}</span><span className="opacity-50">сообщения · воспоминания · подписи скриншотов</span></div>
        : <div className="sticky top-[72px] z-20 mt-3 flex items-center justify-between rounded-2xl border border-black/10 bg-white/95 p-2 shadow-lg backdrop-blur md:top-3">
          <button type="button" disabled={page === 0 || loadingRows} onClick={() => changePage(page - 1)} className="rounded-xl border px-3 py-2 text-xs disabled:opacity-30">Назад</button>
          <span className="text-center text-xs text-burgundy"><span className="font-medium">Страница {page + 1}</span><span className="mt-0.5 block text-[9px] opacity-45">после перехода список сам вернётся наверх</span></span>
          <button type="button" disabled={!nextBoundary || loadingRows} onClick={() => changePage(page + 1)} className="rounded-xl border px-3 py-2 text-xs disabled:opacity-30">Дальше</button>
        </div>}
      {loadError && <div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700">{loadError} <button type="button" onClick={() => void load()} className="ml-2 underline">Повторить</button></div>}
            ...
            <div key={row.id}>
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <a href={`/?preview=1&element=${encodeURIComponent(row.id)}`} target="_blank" rel="noreferrer" className="rounded-lg border border-burgundy/15 bg-white px-3 py-2 text-xs text-burgundy"><Eye size={13} className="mr-1 inline"/>Открыть здесь в Reader</a>
                  <button
                    disabled={bulkBusy}
      ...
      {!q && <div className="mt-5 flex items-center justify-between text-xs">
        <button
          disabled={page === 0}
...
function MemoriesPanel({ specialOnly }: { specialOnly: boolean }) {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [previewIds, setPreviewIds] = useState<Map<string, string>>(new Map());
  const [bulkImportance, setBulkImportance] = useState(3);
  const [bulkAnimation, setBulkAnimation] = useState("fade");
  const [bulkBusy, setBulkBusy] = useState(false);
  const empty = {
    title: "",
    body: "",
  ...
  const visibleRows = useMemo(() => {
    ...
    return rows.filter((row) => `${row.title ?? ""} ${row.body}`.toLocaleLowerCase("ru").includes(normalized));
  }, [query, rows]);
  const load = useCallback(async () => {
    const { data } = await supabase
      .from("memories")
    ...
    if (ids.length) {
      const { data: elements } = await supabase.from("timeline_elements").select("id,memory_id").in("memory_id", ids);
      setPreviewIds(new Map((elements ?? []).flatMap((element) => element.memory_id ? [[String(element.memory_id), String(element.id)] as const] : [])));
    } else setPreviewIds(new Map());
  }, [specialOnly]);
  useEffect(() => {
    void load();
  }, [load]);
  ...
  async function applyBulkMemoryChanges() {
    const targets = rows.filter((row) => selected.has(row.id));
    if (targets.length === 0) return;
    setBulkBusy(true);
    setMessage("");
    try {
      const results = await Promise.all(targets.map((row) => supabase.from("memories").update({
        importance: bulkImportance,
        style: { ...(row.style ?? {}), animation: bulkAnimation },
      }).eq("id", row.id)));
      const error = results.find((result) => result.error)?.error;
      if (error) throw error;
      setMessage(`Обновлено: ${targets.length}.`);
      setSelected(new Set());
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Массовое изменение не удалось.");
    } finally {
      setBulkBusy(false);
    }
  }
  async function removeSelectedMemories() {
    const targets = rows.filter((row) => selected.has(row.id));
    if (targets.length === 0 || !window.confirm(`Удалить выбранные записи (${targets.length})?`)) return;
    setBulkBusy(true);
    try {
      const { error } = await supabase.from("memories").delete().in("id", targets.map((row) => row.id));
      if (error) throw error;
      const paths = targets.map((row) => row.photo_storage_path).filter((path): path is string => Boolean(path));
      if (paths.length) await supabase.storage.from("screenshots").remove(paths);
      setSelected(new Set());
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Удаление не удалось.");
    } finally {
      setBulkBusy(false);
    }
  }
  return (
    <section className="grid gap-5 lg:grid-cols-[.85fr_1.15fr]">
      <form
        ...
        className="scroll-mt-20 rounded-2xl border border-black/10 bg-white p-5 shadow-sm"
      >
        <h1 className="font-serif text-3xl text-burgundy">
      ...
      <div className="space-y-3">
        <div className="sticky top-[72px] z-20 rounded-2xl border border-black/10 bg-white/95 p-3 shadow-lg backdrop-blur md:top-3">
          <label className="flex items-center gap-2 rounded-xl border px-3"><Search size={14} className="opacity-40"/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Найти по заголовку или тексту…" className="min-w-0 flex-1 border-0 py-2 text-sm outline-none"/>{query && <button type="button" onClick={() => setQuery("")}><X size={13}/></button>}</label>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs"><button type="button" onClick={() => setSelected(new Set(visibleRows.map((row) => row.id)))} className="rounded-lg border px-3 py-2">Выбрать найденные</button><button type="button" onClick={() => setSelected(new Set())} className="rounded-lg px-3 py-2 opacity-55">Снять выбор</button><span className="ml-auto text-burgundy">{visibleRows.length} записей · выбрано {selected.size}</span></div>
          {selected.size > 0 && <div className="mt-2 grid gap-2 rounded-xl bg-burgundy/[.04] p-2 sm:grid-cols-[1fr_1fr_auto_auto] sm:items-end"><label className="text-[10px] uppercase tracking-[1px] opacity-55">Важность<select value={bulkImportance} onChange={(event) => setBulkImportance(Number(event.target.value))} className="mt-1 w-full rounded-lg border bg-white p-2 text-xs normal-case">{[0,1,2,3,4,5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label className="text-[10px] uppercase tracking-[1px] opacity-55">Анимация<select value={bulkAnimation} onChange={(event) => setBulkAnimation(event.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 text-xs normal-case"><option value="fade">Плавно</option><option value="float">Парение</option><option value="none">Без анимации</option></select></label><button type="button" disabled={bulkBusy} onClick={() => void applyBulkMemoryChanges()} className="rounded-lg bg-burgundy px-3 py-2 text-xs text-white disabled:opacity-40">Применить</button><button type="button" disabled={bulkBusy} onClick={() => void removeSelectedMemories()} className="rounded-lg border border-red-200 px-3 py-2 text-xs text-red-700 disabled:opacity-40">Удалить</button></div>}
        </div>
        {visibleRows.map((row) => (
          <article
            key={row.id}
            className={`rounded-2xl border bg-white p-5 shadow-sm ${selected.has(row.id) ? "border-burgundy ring-2 ring-burgundy/10" : "border-black/10"}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 items-start gap-3">
                <input type="checkbox" aria-label="Выбрать запись" checked={selected.has(row.id)} onChange={() => toggleSelected(row.id)} className="mt-1 h-4 w-4 shrink-0 accent-burgundy"/>
                <div>
            ...
            <div className="mt-4 flex gap-2">
              ...
              {previewIds.get(row.id) && <a href={`/?preview=1&element=${encodeURIComponent(previewIds.get(row.id) ?? "")}`} target="_blank" rel="noreferrer" className="rounded-lg border px-3 py-2 text-xs text-burgundy"><Eye size={13} className="mr-1 inline"/>Reader</a>}
              <button
...
function ScreenshotsPanel() {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [previewIds, setPreviewIds] = useState<Map<string, string>>(new Map());
  const [bulkFrame, setBulkFrame] = useState("phone");
  const [bulkAnimation, setBulkAnimation] = useState("fade");
  const [bulkPosition, setBulkPosition] = useState("custom");
  const [bulkLayout, setBulkLayout] = useState("carousel");
  const [bulkBusy, setBulkBusy] = useState(false);
  ...
  const [message, setMessage] = useState("");
  const rootRows = useMemo(() => rows.filter((row) => !row.collection_id || row.collection_order === 0), [rows]);
  const visibleRows = useMemo(() => {
    const normalized = query.trim().toLocaleLowerCase("ru");
    if (!normalized) return rootRows;
    return rootRows.filter((row) => `${row.title ?? ""} ${row.description ?? ""} ${row.caption ?? ""}`.toLocaleLowerCase("ru").includes(normalized));
  }, [query, rootRows]);
  const load = async () => {
    const { data } = await supabase
      .from("screenshots")
      ...
      .order("occurred_at", { ascending: false });
    const loaded = (data ?? []) as ScreenshotRow[];
    setRows(loaded);
    const ids = loaded.map((row) => row.id);
    if (ids.length) {
      const { data: elements } = await supabase.from("timeline_elements").select("id,screenshot_id").in("screenshot_id", ids);
      setPreviewIds(new Map((elements ?? []).flatMap((element) => element.screenshot_id ? [[String(element.screenshot_id), String(element.id)] as const] : [])));
    } else setPreviewIds(new Map());
  };
  useEffect(() => {
    void load();
  ...
  function selectedScreenshotRows() {
    const roots = rootRows.filter((row) => selected.has(row.id));
    const collectionIds = new Set(roots.map((row) => row.collection_id).filter((id): id is string => Boolean(id)));
    return rows.filter((row) => selected.has(row.id) || Boolean(row.collection_id && collectionIds.has(row.collection_id)));
  }
  async function applyBulkScreenshots() {
    const targets = selectedScreenshotRows();
    if (targets.length === 0) return;
    setBulkBusy(true);
    setMessage("");
    try {
      const results = await Promise.all(targets.map((row) => supabase.from("screenshots").update({
        animation: bulkAnimation,
        position: bulkPosition,
        collection_layout: bulkLayout,
        style: { ...(row.style ?? {}), frame: bulkFrame },
      }).eq("id", row.id)));
      const error = results.find((result) => result.error)?.error;
      if (error) throw error;
      setMessage(`Обновлено файлов: ${targets.length}.`);
      setSelected(new Set());
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Массовое изменение не удалось.");
  ...
  async function removeSelectedScreenshots() {
    const targets = selectedScreenshotRows();
    if (targets.length === 0 || !window.confirm(`Удалить выбранные скриншоты и альбомы (${targets.length} файлов)?`)) return;
    setBulkBusy(true);
    setMessage("");
    try {
      const { error } = await supabase.from("screenshots").delete().in("id", targets.map((row) => row.id));
      if (error) throw error;
      const paths = targets.map((row) => row.storage_path).filter(Boolean);
      if (paths.length) await supabase.storage.from("screenshots").remove(paths);
      setSelected(new Set());
      await load();
    } catch (error) {
  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="space-y-3">
        <div className="sticky top-[72px] z-20 rounded-2xl border border-black/10 bg-white/95 p-3 shadow-lg backdrop-blur md:top-3">
          <label className="flex items-center gap-2 rounded-xl border px-3"><Search size={14} className="opacity-40"/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Найти скриншот или подпись…" className="min-w-0 flex-1 border-0 py-2 text-sm outline-none"/>{query && <button type="button" onClick={() => setQuery("")}><X size={13}/></button>}</label>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs"><button type="button" onClick={() => setSelected(new Set(visibleRows.map((row) => row.id)))} className="rounded-lg border px-3 py-2">Выбрать найденные</button><button type="button" onClick={() => setSelected(new Set())} className="rounded-lg px-3 py-2 opacity-55">Снять выбор</button><span className="ml-auto text-burgundy">{visibleRows.length} карточек · выбрано {selected.size}</span></div>
          {selected.size > 0 && <div className="mt-2 grid gap-2 rounded-xl bg-burgundy/[.04] p-2 sm:grid-cols-2 xl:grid-cols-[repeat(4,1fr)_auto_auto] xl:items-end"><label className="text-[10px] uppercase tracking-[1px] opacity-55">Рамка<select value={bulkFrame} onChange={(event) => setBulkFrame(event.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 text-xs normal-case"><option value="phone">Телефон</option><option value="polaroid">Полароид</option><option value="soft">Мягкая</option><option value="none">Без рамки</option></select></label><label className="text-[10px] uppercase tracking-[1px] opacity-55">Анимация<select value={bulkAnimation} onChange={(event) => setBulkAnimation(event.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 text-xs normal-case"><option value="fade">Плавно</option><option value="float">Парение</option><option value="none">Без анимации</option></select></label><label className="text-[10px] uppercase tracking-[1px] opacity-55">Позиция<select value={bulkPosition} onChange={(event) => setBulkPosition(event.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 text-xs normal-case"><option value="custom">Свободная</option><option value="after_message">После сообщения</option><option value="before_message">Перед сообщением</option><option value="after_date">После даты</option></select></label><label className="text-[10px] uppercase tracking-[1px] opacity-55">Альбом<select value={bulkLayout} onChange={(event) => setBulkLayout(event.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 text-xs normal-case"><option value="carousel">Карусель</option><option value="stack">Стопка</option><option value="collage">Коллаж</option></select></label><button type="button" disabled={bulkBusy} onClick={() => void applyBulkScreenshots()} className="rounded-lg bg-burgundy px-3 py-2 text-xs text-white disabled:opacity-40">Применить</button><button type="button" disabled={bulkBusy} onClick={() => void removeSelectedScreenshots()} className="rounded-lg border border-red-200 px-3 py-2 text-xs text-red-700 disabled:opacity-40">Удалить</button></div>}
        </div>
        {visibleRows.map((row) => (
          <article
            key={row.id}
            className={`rounded-2xl border bg-white p-5 shadow-sm ${selected.has(row.id) ? "border-burgundy ring-2 ring-burgundy/10" : "border-black/10"}`}
          >
            <div className="flex items-start gap-3"><input type="checkbox" aria-label="Выбрать скриншот или альбом" checked={selected.has(row.id)} onChange={() => toggleSelected(row.id)} className="mt-0.5 h-4 w-4 shrink-0 accent-burgundy"/><div className="min-w-0 flex-1"><div className="text-xs opacity-45">
