...
export default function AnalyticsPanel() {
  ...
  return <section className="space-y-5">
    ...
    {reactions.length > 0 && <div className="rounded-[26px] border border-black/5 bg-white/85 p-5 shadow-sm"><div className="flex items-center gap-2"><Heart size={17} className="text-burgundy/55" /><h2 className="font-serif text-2xl text-burgundy">Её реакции и мнение</h2></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{reactions.slice(0, 40).map((reaction) => <div key={reaction.id} className="min-w-0 rounded-2xl border border-burgundy/10 bg-[#FBF8F5] px-4 py-3"><div><span className="text-xl">{reaction.emoji}</span><span className="ml-2 text-[10px] opacity-45">{when(reaction.updated_at)}</span></div>{reaction.note && <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-burgundy">«{reaction.note}»</p>}<div className="mt-2 truncate text-[9px] opacity-30">страница {reaction.element_id.slice(0, 8)}</div></div>)}</div></div>}
